...
public class TulLentAdapter extends RecyclerView.Adapter<TulLentAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(final ViewHolder holder, int position) {
        ...
        if (holder.Holderid == 0) {
            ...
            holder.tvTulHanded.setText(R.string.Mark_Check_In);
            holder.tvTulReceived.setText(R.string.Mark_Check_Out);
            holder.tvOwner.setText(R.string.RENTED);
            ...
            if (mData.get(position).getAdditional_charges() != null && !android.text.TextUtils.isEmpty(mData.get(position).getAdditional_charges().getFee())) {
                transactionFee = ((Double.parseDouble(mData.get(position).getBase_price().replace(",","."))
                        + Double.parseDouble(mData.get(position).getAdditional_charges().getFee().replace(",",".")))
                        * Double.parseDouble(mData.get(position).getTransaction_percentage().replace(",","."))) / 100;
            } else {
                transactionFee = (Double.parseDouble(mData.get(position).getBase_price().replace(",",".")) * Double.parseDouble(mData.get(position).getTransaction_percentage())) / 100;
            }
            double earnedAmount = 0.0;

            double securityFee;
            if (!TextUtils.isEmpty(mData.get(position).getAdditional_charges().getSecurity_charges().replace(",",".")))
                securityFee = Double.parseDouble(mData.get(position).getAdditional_charges().getSecurity_charges().replace(",","."));
            else
                securityFee = 0.0;

            earnedAmount = Double.parseDouble(mData.get(position).getTotal_amount().replace(",","."))
                    - (transactionFee + securityFee);
            ...
            try {
                if (mData.get(position).getLender_status() == 0) {
                    ...
                    holder.tvTulHandedDate.setText(Constants.convertUTCtoGMT(mData.get(position).getHandover_at()));
                    ...
                    holder.tvtulRecievedDate.setText(Constants.convertUTCtoGMT(mData.get(position).getLander_received_at()));
                }
            } catch (Exception e) {
                Log.e("Exce  = ", e + "");
            }
...
//                                    String.valueOf(mData.get(finalPosition).getBooking_id()), finalPosition);
//                        else
//                            Toast.makeText(mContext, "You can't checked in before booking date.", Toast.LENGTH_SHORT).show();
//                    }
                }
            });
            ...
            holder.llTulReceived.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    if (mData.get(finalPosition).getLender_status() == 1) {
                        ...
                        Toast.makeText(mContext, mContext.getString(R.string.You_cannot_check_out_before_the_scheduled_booking_date_and_time), Toast.LENGTH_SHORT).show();
                    }
                }
            });
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public ViewHolder(View itemView, int viewType) {
            ...
            if (viewType == TYPE_HEADER) {
                ...
                llTulLent.setBackground(ContextCompat.getDrawable(mContext, R.drawable.white_color_stroke));
