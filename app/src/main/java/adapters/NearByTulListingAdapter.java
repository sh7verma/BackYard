...
import com.app.backyard.ListingTulDetailActivity;
...
public class NearByTulListingAdapter extends RecyclerView.Adapter<NearByTulListingAdapter.ViewHolder> {
    ...
    public NearByTulListingAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType == TYPE_ITEM) {
            ...
            return vhItem = new NearByTulListingAdapter.ViewHolder(v, viewType);
        }
    }
    ...
    public void onBindViewHolder(final NearByTulListingAdapter.ViewHolder holder, final int position) {
        if (holder.Holderid == 1) {
            ...
            holder.txtPrice.setText(mData.get(position).getCurrency() + Double.parseDouble(mData.get(position).getPrice()));

            if (!TextUtils.isEmpty(mData.get(position).getAttachment().get(0).getAttachment()) && mData.get(position).getAttachment().get(0).getType().equals(Constants.IMAGE_TEXT)) {
                ...
                Picasso.with(con).load(mData.get(position).getAttachment().get(0).getAttachment())
                        ...
                        .into(holder.imgTul);
            } else if (!TextUtils.isEmpty(mData.get(position).getAttachment().get(0).getThumbnail()) && mData.get(position).getAttachment().get(0).getType().equals(Constants.VIDEO_TEXT)) {
            ...
            Picasso.with(con).load(mData.get(position).getOwner_pic())
                    ...
                    .centerCrop().placeholder(R.mipmap.ic_small_ph)
                ...
                public void onClick(View v) {
                    if (utils.getInt(Constants.ISGUEST, 0) == 0) {
                        ...
                        if ((new Connection_Detector(con)).isConnectingToInternet()) {
                            ...
                            Toast.makeText(con,  con.getResources().getString(R.string.Internet_connection_not_available), Toast.LENGTH_SHORT).show();
                        }
                    } else {
                        loginFirst();
                    }
            ...
            holder.llRating.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    intent.putExtra("type", mData.get(position).getProduct_type());
   ...
   private void isChatDisabled(int id, final String dialogueId, final NearByTulListingModel.ResponseBean responseBean, final DataSnapshot dataSnapshot) {
