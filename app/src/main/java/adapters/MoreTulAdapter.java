...
public class MoreTulAdapter extends RecyclerView.Adapter<MoreTulAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        ...
        holder.tvPrice.setText(Constants.POUND + mMoreTulArrayList.get(position).getPrice());

        Picasso.with(con)
                .load(mMoreTulArrayList.get(position).getAttachment().get(0).getThumbnail())
                .resize(mHeight / 4, mHeight / 5).placeholder(R.drawable.gradient_base)
                .centerCrop().into(holder.imgTul);
