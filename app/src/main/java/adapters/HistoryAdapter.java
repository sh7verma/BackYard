...
import com.app.backyard.HistoryTulDetailActivity;
        ...
        if (holder.Holderid == 1) {
            if (mFragment == 1) {
                ...
                Picasso.with(con).load(mData.get(position).getOwner_pic())
                        .resize(mProfileImageDimen, mProfileImageDimen).centerCrop().placeholder(R.mipmap.ic_add_image)
                        .transform(new CircleTransform())
                        .into(holder.imgProfile);
            } else {
                holder.txtOwner.setText("RENTED TO");
