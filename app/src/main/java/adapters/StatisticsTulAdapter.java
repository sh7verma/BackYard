...
public class StatisticsTulAdapter extends RecyclerView.Adapter<StatisticsTulAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(final ViewHolder holder, int position) {
        if (holder.Holderid == 1) {
            ...
            holder.tvPrice.setText(utils.getCurrency(utils.getString(Constants.PRIMARY_CURRENCY,"")) + mData.get(position).getPrice());

            if (mData.get(position).getTotal_bookings() == 0)
                holder.tvBookedTul.setText(R.string.Not_booked_yet);
            else
                holder.tvBookedTul.setText(String.valueOf(mData.get(position).getTotal_bookings()));

            holder.tvTotalEarnings.setText(utils.getCurrency(utils.getString(Constants.PRIMARY_CURRENCY,"")) + mData.get(position).getTotal_earning());
