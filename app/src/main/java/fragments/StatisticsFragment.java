...
public class StatisticsFragment extends Fragment implements View.OnClickListener {
    ...
    public void initUI() {
        ...
        if ((new Connection_Detector(mContext)).isConnectingToInternet()) {
            ...
            Toast.makeText(mDashBoardActivity, R.string.internet, Toast.LENGTH_SHORT).show();
            txtNoTul.setVisibility(View.VISIBLE);
            txtNoTul.setText("No Internet Connection");
    ...
    public void moveToDetail(int position) {
        if ((new Connection_Detector(mContext)).isConnectingToInternet()) {
            Intent intent = new Intent(mContext, BackYardDetailActivity.class);
            intent.putExtra("position", position);
            intent.putExtra("tul_id", mDashboardTul.get(position).getTool_id());
            startActivityForResult(intent, TULDETAIL);
        } else {
            Snackbar.make(rvListing, "Internet connection not available!", Snackbar.LENGTH_SHORT).show();
        }
    }
