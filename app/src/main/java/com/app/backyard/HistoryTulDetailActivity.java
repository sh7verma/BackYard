...
public class HistoryTulDetailActivity extends BaseActivity implements AppBarLayout.OnOffsetChangedListener {
    ...
    protected void initUI() {
        ...
        llTulName.setPadding(mWidth / 32, mHeight / 32, mWidth / 32, mHeight / 32);
        ...
        mAdapter.addFragment(new HistoryHelpFragment(), "HELP");
        mAdapter.addFragment(new HistoryReceiptFragment(), "RECEIPT");
    ...
    protected void onCreateStuff() {
        ...
        tvTulPrice.setText(mTulModel.getCurrency() +String.format("%.2f", Double.parseDouble(mTulModel.getPrice())));
        srbRating.setRating(mTulModel.getRating());
        try {
            if (mTulModel.getBorrower_status() == 3 || mTulModel.getLender_status() == 3) {
                tvLentTitle.setText(R.string.cancelled_on);
                tvLendDate.setText(Constants.convertUTCtoGMT(mTulModel.getCancelled_at()));
                tvReturnedTitle.setVisibility(View.INVISIBLE);
                tvReturnedDate.setVisibility(View.INVISIBLE);
                llRating.setVisibility(View.GONE);
            } else {
                tvLentTitle.setText(getString(R.string.check_in));
                tvReturnedTitle.setText(getString(R.string.check_out));

                if (mTulModel.getProduct_type().equals("1")) {
                    tvLendDate.setText(Constants.convertDate(mTulModel.getDelivery_date()));
                    tvReturnedDate.setText(Constants.convertDate(mTulModel.getReturn_date()));
                } else if (mTulModel.getProduct_type().equals("0")) {
                    tvLendDate.setText(Constants.convertDate(mTulModel.getDelivery_date()));
                    tvReturnedDate.setText(Constants.addOneDayToDate(mTulModel.getReturn_date()));
                }
