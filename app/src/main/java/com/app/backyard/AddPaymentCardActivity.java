...
public class AddPaymentCardActivity extends BaseActivity {
    ...
    private String mEndTime, mCheckOutTime, mCheckInTime, mTotalPrice, mTotalAmount, mDiscountPrice, mTransactionCharges, mExtraCharges;
    ...
    protected void onCreateStuff() {
        if (getIntent().hasExtra("checkout")) {
            ...
            mTransactionCharges = getIntent().getStringExtra("transaction_fee");
            mTotalPrice = getIntent().getStringExtra("base_price");
            mTotalAmount = getIntent().getStringExtra("tul_price");
            mDiscountPrice = getIntent().getStringExtra("discount_price");
            mExtraCharges = getIntent().getStringExtra("extra_charges");

            if (TextUtils.isEmpty(mExtraCharges)) {
                mExtraCharges = "0.00";
            }
            if (TextUtils.isEmpty(mTransactionCharges)) {
                mTransactionCharges = "0.00";
            }
            if (TextUtils.isEmpty(mDiscountPrice)) {
                mDiscountPrice = "0.00";
            }

            tvBtn.setText(getString(R.string.proceed_to_pay));
            tvBtn.setAllCaps(true);
        } else {
            txtToolbarTitle.setText(getString(R.string.add_new_card));
            txtToolbarTitle.setAllCaps(true);
        }
    ...
    private void verifyCardDetails() {
        if (TextUtils.isEmpty(edNameOnCard.getText().toString().trim()))
            ...
            showAlert(llInner, getString(R.string.invalid_cvv_number));
        else {
            String cardNo = "";
            try {
            ...
            if (mSaveCard && db.getCard(cardNo.substring(0, cardNo.length() - 1)) != 0) {
                showAlert(llInner, getString(R.string.You_already_saved_this_card_earlier));
            } else {
                if (connectedToInternet()) {
                    ...
                    if (!mStripeCard.validateCVC()) {
                        ...
                        showAlert(llInner, getString(R.string.Invalid_card));
                    } else {
                        AlertDialog.Builder builder = new AlertDialog.Builder(this);
                        builder.setMessage(R.string.Are_you_sure_you_want_to_make_this_payment)
                                .setCancelable(false)
    ...
    private void hitBookAPI(String token) {
        ...
        if (mSelectedDates.size() > 0)
            ...
            returndate = mSelectedDates.get(0) + " " + mEndTime;
        ...
        if (mViewTulModel.getProduct_type() == 1) {
            deliverydate = mSelectedDates.get(0) + " " + mCheckInTime;
            returndate = mSelectedDates.get(0) + " " + mCheckOutTime;
        }

        if (mViewTulModel.getProduct_type() == 0) {
            mCheckInTime = mViewTulModel.getPreferences().getStart_time();
            mCheckOutTime = mViewTulModel.getPreferences().getEnd_time();
        }

        if (TextUtils.isEmpty(additionalPriceBean.getSecurity_charges())) {
            additionalPriceBean.setSecurity_charges("0.00");
        }
        ...
        additionalPrice.addProperty("discount_percentage", additionalPriceBean.getDiscount_percentage());
        additionalPrice.addProperty("discount_days", additionalPriceBean.getDiscount_days());
        additionalPrice.addProperty("price", additionalPriceBean.getPrice());

        Call<BookTulModel> call = RetrofitClient.getInstance().bookATul(utils.getString("access_token", ""),
                mViewTulModel.getId(), additionalPrice.toString(), mViewTulModel.getPrice(), String.format("%.2f", Double.parseDouble(mTotalPrice)),
                deliverydate, returndate,
                additionalPriceBean.getSecurity_charges(),
                String.format("%.2f", Double.parseDouble(mTotalAmount)),
                mViewTulModel.getAddress(), getIntent().getStringExtra("latitude"),
                getIntent().getStringExtra("longitude"), selectedDates,
                token, deliverytype, getIntent().getStringExtra("delivery_charges"),
                getIntent().getStringExtra("quantity"), additionalPriceBean.getFee(),
                savecard, cardNo,
                edNameOnCard.getText().toString().trim(), tvExpiryMonth.getText().toString(),
                tvExpiryYear.getText().toString(), mGson.toJson(mViewTulModel.getPreferences()), mViewTulModel.getAddress(), mCheckInTime, mCheckOutTime,
                String.format("%.2f", Double.parseDouble(mDiscountPrice)),
                String.format("%.2f", Double.parseDouble(mTransactionCharges)),
                String.format("%.2f", Double.parseDouble(mExtraCharges)), additionalPriceBean.getDiscount_days(), additionalPriceBean.getDiscount_percentage(), getIntent().getStringExtra("buffer_available"));
        call.enqueue(new Callback<BookTulModel>() {
            ...
            public void onResponse(Call<BookTulModel> call, Response<BookTulModel> response) {
                ...
                if (response.body().getResponse() != null) {
                    ...
                    db.addMyBooking(response.body().getResponse());
