...
//                selectOptions(getCountry(), COUNTRY, txtCountryCodeValue.getText().toString());
            ...
            case R.id.txt_save:
                if (TextUtils.isEmpty(txtCountryCodeValue.getText().toString().trim()))
                    ...
                    showAlert(llInside, getString(R.string.erroe_valid_account_number));
...
//        body = MultipartBody.Part.createFormData("document", mFile.getName(), reqFile);
        ...
        RequestBody access_token = RequestBody.create(MediaType.parse("text/plain"), utils.getString("access_token", "b9c937f9d182845466c952b8f5f7f0bc"));
...
//        RequestBody country_code = RequestBody.create(MediaType.parse("text/plain"), txtCountryCodeValue.getText().toString());
        ...
        Call<CreateStripeAccountModel> call = RetrofitClient.getInstance().stripeBank(access_token, country_code, currency
                , account_number, first_name, last_name, address, city, state, postal_code, dob, sort_code, swift, account_type, body);
