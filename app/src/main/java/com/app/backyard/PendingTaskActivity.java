...
public class PendingTaskActivity extends BaseActivity {
    ...
    public void setData() {
       ...
       /*type 1 if lender cancel booking
        ...
        if (mPendingModels.getType() == 1) {/// cancelled by lender.
            ...
            tvNotReceived.setText(R.string.CANCELLED);
            ...
            llButton.setVisibility(View.GONE);
            txtDone.setVisibility(View.VISIBLE);
            tvOwner.setText(R.string.Borrower);
            displayCancelled();
            ...
            tvNotReceived.setText(R.string.RETURNED);
            llButton.setVisibility(View.VISIBLE);
            txtDone.setVisibility(View.GONE);
            btReceived.setText(R.string.RECEIVED);
            btNotReceived.setText(R.string.NOT_RECEIVED);
            tvOwner.setText(R.string.Borrower);
            tvOwnerName.setText(mPendingModels.getBorrower());
        } else if (mPendingModels.getType() == 4) {/// Handedover by lender section
            /// visible on borrower side
            tvNotReceived.setText(R.string.HANDED_OVER);
            llButton.setVisibility(View.VISIBLE);
            txtDone.setVisibility(View.GONE);
            tvOwnerName.setText(mPendingModels.getOwner());
        }
        ...
        try {
            tvDate.setText(Constants.convertDate(mPendingModels.getDelivery_date()) + " -  " + Constants.addOneDayToDate(mPendingModels.getReturn_date()));
        } catch (Exception e) {
            Log.e("Date Exce  = ", e + "");
        }
    }
    ...
    private void displayCancelled() {
        ...
        tvSign.setText("This Place booking has been cancelled.");
    ...
    private void onNotReceivedClicked() {
        if (isSigned) {
            ...
            if (bitmap != null) {
                ...
                showAlert(llMain, "Please Sign above");
            }
        } else
            showAlert(llMain, "Please Sign above");
    }
...
//                    Toast.makeText(mContext, response.body().response.getMessage(), Toast.LENGTH_SHORT).show();
    ...
    private void alertPOPUP(int path) {
        String message;
        if (path == 1)
            message = "Do you want to report or contact owner?";
        else
            message = "Do you want to report or contact borrower?";
...
//                    Toast.makeText(mContext, response.body().response.getMessage(), Toast.LENGTH_SHORT).show();
                    ...
                    intent.putExtra("totalAmount", String.valueOf(mPendingModels.getTotal_amount()));
