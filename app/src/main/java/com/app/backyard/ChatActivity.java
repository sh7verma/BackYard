...
//            Toast.makeText(mChatActivity, "You have been unmatched", Toast.LENGTH_LONG).show();
    ...
    private void setProfileData() {
        ...
        if (!TextUtils.isEmpty(mChatDialogModel.getProfile_pic().get(particpantId)))
            Picasso.with(mContext).load(mChatDialogModel.getProfile_pic().get(particpantId)).resize((mWidth / 7), (mWidth / 7))
                    .centerCrop()
                    ...
                    if (mOtherBlockStatus == 0) {
                        ...
                        if (finalMessage.length() < 2000) {
                            if (finalMessage.length() > 0) {
                                ...
                                if (mChatDialogModel.getDelete_outer_dialog().get(utils.getString("user_id", "")).equals("1") || mChatDialogModel.getDelete_outer_dialog().get(particpantId).equals("1")) {

                                    FirebaseDatabase.getInstance().getReference().child(Constants.CHAT_ENDPOINT)
                                            .child(mChatDialogModel.getChat_dialog_id()).child("delete_outer_dialog")
                            ...
                            Toast.makeText(ChatActivity.this, R.string.max_character_limit_is_2000, Toast.LENGTH_SHORT).show();
                        }
                    } else {
                        unBlockDialog();
                    }
...
//                    else if (data.getIntExtra("UNMATCH", 0) == UNMATCH) {
//                        unMatch();
//                    } else if (data.getStringExtra("mute_status").equals(Constants.UNMUTE)) {
//                        /// set mute status of own user to unmute
//                        FirebaseDatabase.getInstance().getReference().child(Constants.CHAT_ENDPOINT)
//                                .child(mChatDialogModel.getChat_dialog_id()).child("mute").child(utils.getString("user_id", ""))
//                                .setValue(Constants.UNMUTE);
//
////                        Map<String, String> tempMap7 = new HashMap<>();
////                        tempMap7.put(utils.getString("user_id", ""), Constants.UNMUTE);
////                        tempMap7.put(mOtherUserID, Constants.UNMUTE);
////                        mChatDialogModel.setMute(tempMap7);
//
//                    }
//                    else if (data.getStringExtra("mute_status").equals(Constants.MUTE)) {
//                        /// set mute status of own user to mute
//                        FirebaseDatabase.getInstance().getReference().child(Constants.CHAT_ENDPOINT)
//                                .child(mChatDialogModel.getChat_dialog_id()).child("mute").child(utils.getString("user_id", ""))
//                                .setValue(Constants.MUTE);
//
////                        Map<String, String> tempMap7 = new HashMap<>();
////                        tempMap7.put(utils.getString("user_id", ""), Constants.MUTE);
////                        tempMap7.put(mOtherUserID, Constants.MUTE);
////                        mChatDialogModel.setMute(tempMap7);
//                    }
                    break;
            }
    ...
    private void unMatch() {
        FirebaseDatabase.getInstance().getReference().child(Constants.USER_ENDPOINT).child(utils.getString("user_id", ""))
                .child("chat_dialog_ids").child(mChatDialogModel.getChat_dialog_id()).removeValue();

        FirebaseDatabase.getInstance().getReference().child(Constants.USER_ENDPOINT).child(mOtherUserID)
                .child("chat_dialog_ids").child(mChatDialogModel.getChat_dialog_id()).removeValue();

        FirebaseDatabase.getInstance().getReference().child(Constants.CHAT_ENDPOINT)
                .child(mChatDialogModel.getChat_dialog_id()).removeValue();

        FirebaseDatabase.getInstance().getReference().child(Constants.MESSAGES_ENDPOINT)
                .child(mChatDialogModel.getChat_dialog_id()).removeValue();

//        unMatchUser(mChatDialogModel.getChat_dialog_id(), mOtherUserID);
    }
...
//        ShortcutBadger.removeCount(con);
    ...
    private void optionDialog() {
        ...
        if (mOtherBlockStatus == 0) {
            txtBlock.setText(getResources().getString(R.string.block));
        } else {
            txtBlock.setText(getString(R.string.unblock));
        }
    ...
    void blockDialog() {
        ...
        alertDialog.setMessage(R.string.Are_you_sure_you_want_to_block_this_User);
    ...
    void unBlockDialog() {
        AlertDialog.Builder alertDialog = new AlertDialog.Builder(ChatActivity.this);
        alertDialog.setMessage(R.string.You_need_to_unblock_this_user_to_send_a_message);
        alertDialog.setPositiveButton(getString(R.string.unblock), new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int which) {
    ...
    void clearDialog() {
        AlertDialog.Builder alertDialog = new AlertDialog.Builder(ChatActivity.this);
        alertDialog.setMessage(R.string.Are_you_sure_you_want_to_clear_this_conversation);
        alertDialog.setPositiveButton(getString(R.string.clear), new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int which) {
                clearChat();
                dialog.dismiss();
