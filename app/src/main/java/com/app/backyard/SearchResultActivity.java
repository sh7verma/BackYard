...
public class SearchResultActivity extends BaseActivity implements OnMapReadyCallback,
    ...
    protected void onCreateStuff() {
        ...
        if (Constants.FILTER_COUNT == 1)
            ...
            txtFilter.setText(getString(R.string.Filter_Applied));

        if (ContextCompat.checkSelfPermission(this, Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M)
                requestPermissions(new String[]{Manifest.permission.ACCESS_FINE_LOCATION}, LOCATION_CHECK);
        } else {
    ...
    private void alertDialogClear() {
        ...
        builder.setMessage(R.string.Are_you_sure_you_want_to_clear_filters)
                .setCancelable(false)
...
//        startActivityForResult(inLoc, LOCATION);
    ...
    private void hitAPI(String latitude, String longitude) {
        ...
        if (Constants.BEST_RATED)
            bestRated = 1;
        String amenites = Constants.AMENITIES.replace(Constants.RULES_REGEX, Constants.RULES_REGEX_RESPONSE);

        Call<SearchResultModel> call = RetrofitClient.getInstance().searchTul(utils.getString("access_token", ""),
                Constants.TITLE, Constants.CATEGORY_ID, amenites, Constants.MAX_PRICE_SEARCH,
                Constants.MIN_PRICE_SEARCH, Constants.AVAILABILITY, String.valueOf(latitude), String.valueOf(longitude), String.valueOf(Constants.PRODUCT_TYPE), String.valueOf(Constants.DISTANCE), bestRated);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (ContextCompat.checkSelfPermission(SearchResultActivity.this, Manifest.permission.ACCESS_FINE_LOCATION) == PackageManager.PERMISSION_GRANTED) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M)
    ...
    private class TulRender extends DefaultClusterRenderer<SearchResultModel.ResponseBean> {
        ...
        protected void onBeforeClusterRendered(Cluster<SearchResultModel.ResponseBean> cluster, MarkerOptions markerOptions) {
            ...
            txtTitleCluster.setText("PLACES");
